...
def extract_scalars(csv_path):
    ...
    # [v33.22] cog_decel — Driveline 검증 채택 변수
    #   Driveline 정의: peak forward velocity − velocity at release (m/s)
    #   소스: whole_body_center_of_mass (3D 속도, KH-30 ~ BR+5 구간)
    #   회귀계수: +3.282 km/h per m/s (β=+0.183, p=0.008, ΔLOO-CV +0.021)
    #   코호트 평균 1.02 ± 0.45 m/s, Driveline 엘리트 1.61 m/s
    if events['fc'] is not None and events['br'] is not None:
        lo = max(0, events['fc'] - 30)
        hi = events['br'] + 5
        ri_lo, ri_hi = td._row_range(lo, hi)
        if ri_lo is not None and all(c in td.cols for c in
                                       ['whole_body_center_of_mass_x', 'whole_body_center_of_mass_y', 'whole_body_center_of_mass_z']):
            sub = td.df.iloc[ri_lo:ri_hi+1]
            cx = sub['whole_body_center_of_mass_x'].apply(safe_num).values
            cy = sub['whole_body_center_of_mass_y'].apply(safe_num).values
            cz = sub['whole_body_center_of_mass_z'].apply(safe_num).values
            ff = sub['frame'].apply(safe_num).values
            dt = 1 / fps
            vels = []
            for i in range(1, len(cx)):
                ...
                if df_ <= 0:
                    vels.append(float('nan'))
                    continue
                dx = cx[i] - cx[i-1]; dy = cy[i] - cy[i-1]; dz = cz[i] - cz[i-1]
                v = math.sqrt(dx*dx + dy*dy + dz*dz) / (df_ * dt)
                vels.append(v)
            import numpy as _np
            vels_arr = _np.array(vels, dtype=float)
            if _np.any(~_np.isnan(vels_arr)):
                peak_idx = int(_np.nanargmax(vels_arr))
                v_peak = float(vels_arr[peak_idx])
                last_3 = vels_arr[-3:]
                v_end_arr = last_3[~_np.isnan(last_3)]
                if len(v_end_arr) > 0 and peak_idx < len(vels_arr) - 1:
                    v_end = float(v_end_arr.mean())
                    out['cog_decel'] = v_peak - v_end  # m/s
                    out['wb_max_cog_velo'] = v_peak     # whole-body max forward velocity (보조 지표)

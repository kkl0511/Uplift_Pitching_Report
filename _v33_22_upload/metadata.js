...
const EXTRA_VAR_SCORING = {
  ...
  'cog_decel':                    { optimal: 1.5, sigma: 0.45 },                       // m/s — v33.22 신규 (Driveline 검증). peak forward velo − release velo. Driveline 엘리트 1.61, 우리 코호트 1.02±0.45. 회귀계수 +3.28 km/h per m/s, p=0.008
...
const PLAUSIBLE_RANGES = {
  ...
  'cog_decel':                    { min: 0.0, max: 4.0 },       // m/s — v33.22 신규. peak−release CoG velocity 차이. 음수 불가능, 4 이상은 outlier
...
// ════════════════════════════════════════════════════════════════════
// v33.22 (2026-05-07) — VELO_REGRESSION 다중회귀 + Driveline 검증 cog_decel 추가
// ════════════════════════════════════════════════════════════════════
//   v33.21 10변수 + cog_decel 1개 추가 = 11변수 회귀 (n=166)
//   Driveline "Mechanical Composite Scores+" 모델 변수 검증 후 단일 채택:
//     - 채택 기준: ΔLOO-CV > 0.005 AND p < 0.10 AND VIF < 5
//     - cog_decel 통과: ΔLOO-CV +0.021, p=0.008, VIF=6.9 (단독 가치 압도적)
//     - Hip-Shoulder Sep at FP → max_x_factor와 동일 개념 (이미 보유) → 거부
//     - Torso Rotation at FP → Flying Open 결함과 동일 개념 (이미 카드 표시) → 거부
//   R² = 0.377 (v33.21 0.361 대비 +0.016), adj R² = 0.332
//
//   사용법:
...
//     - cog_decel_mean            → mechanics.cog_decel  ★ 신규 (v33.22)
//                                   = peak_forward_velocity - velocity_at_release (m/s)
//                                   추출 위치: extract_uplift_scalars.py (whole_body_center_of_mass)
const VELO_REGRESSION_v33_22 = {
  intercept: 8.6684,
  // 비표준화 회귀계수 (실제 단위 그대로 곱셈)
  coefs: {
    max_x_factor_mean:                  0.167754,  // °     → +1° = +0.17 km/h
    lead_knee_ext_change_fc_to_br_mean: -0.005062,  // °    → 미미 (음수, 다중공선)
    proper_sequence_binary_mean:        9.434136,  // 0~1   → 0→1 = +9.4 km/h ★ 1순위
    pelvis_to_trunk_lag_ms_sd:          0.008663,  // ms    → 미미
    elbow_ext_vel_max_mean:             0.001093,  // °/s   → +100°/s = +0.11 km/h
    height_m:                          42.929610,  // m     → +0.1m = +4.3 km/h ★
    weight_kg:                          0.021681,  // kg    → 미미
    cmj_pp_bm:                          0.065220,  // W/kg  → +10 = +0.65 km/h
    imtp_pp_bm:                         0.235938,  // N/kg  → +5 = +1.2 km/h
    grip_strength:                      0.264108,  // kg    → +5 = +1.3 km/h ★
    cog_decel_mean:                     3.282493,  // m/s   → +0.5m/s = +1.6 km/h ★ 신규
  },
  R2: 0.377,
  adj_R2: 0.332,
  LOO_R2: 0.268,
  n: 166,
  trained: '2026-05-07',
  source: 'BBL 166 sessions × master_fitness MaxV (OLS, +cog_decel from Driveline 검증)',
  validation: {
    'kimgangyeon_H2_2025-11-21': { measured: 146.3, predicted: 135.3, residual: +11.1 },  // v33.21 +12.2 → +11.1
  },
  // 주요 표준화 β (해석용 — 단위 무관 비교)
  std_beta_top: [
    ['proper_sequence_binary', +0.276],  // 1순위
    ['grip',                   +0.253],  // 2순위
    ['height',                 +0.235],  // 3순위
    ['max_x_factor_mean',      +0.214],  // 4순위
    ['cog_decel_mean',         +0.183],  // 5순위 ★ 신규
  ],
};

// v33.21 호환성을 위한 alias (deprecated, 점진 제거)
const VELO_REGRESSION_v33_21 = VELO_REGRESSION_v33_22;
...
function predictMaxVelocity(mech, fit) {
  ...
  const w = VELO_REGRESSION_v33_22.coefs;
  ...
  const cog_decel = safe(mech.cog_decel || mech.cog_decel_mean, 1.02);  // ★ 신규 — 코호트 mean 1.02 m/s
  const v = VELO_REGRESSION_v33_22.intercept
